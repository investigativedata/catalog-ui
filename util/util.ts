...
export function applyActiveFilters(items: IDatasetTransformed[], filters: any) {
  ...
  if (filters.tags.length > 0) {
    filteredItems = filteredItems.filter(({ tags }) => filters.tags.some((t: string) => tags?.includes(t)))
  }

  if (filters.countries.length > 0) {
    filteredItems = filteredItems.filter(({ countries }) => filters.countries.some((c: string) => countries?.find(({ code }) => code === c)))
  }
